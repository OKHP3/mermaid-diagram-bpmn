/**
 * validate-skill.test.mjs — okhp3-visual-process-modeling
 */
...
test('name matches directory', () => {
  assert.equal(parseFrontmatter(read('SKILL.md')).name, 'okhp3-visual-process-modeling');
});
...
test('validateBpmnBeta exports named function', async () => {
  const mod = await import(pathToFileURL(join(SKILL_ROOT, 'scripts/validate-bpmn-beta.mjs')).href);
