...
const UNQUOTED_LABEL_RE = /^[a-zA-Z][a-zA-Z0-9_]*\s+(-->|==>|~~>|pool|lane|start|end|task|xor|and|or)\s+.*[^"]\s*$/;
...
/**
 * @typedef {{ id: string, keyword: string, kind: string, poolId?: string, laneId?: string }} Element
 * @typedef {{ source: string, target: string, kind: string, label?: string, poolContext?: string }} Flow
 * @typedef {{ elements: Element[], flows: Flow[], pools: string[], lanes: string[], errors: string[] }} ParseResult
 */
...
function parseBpmnBeta(source) {
  ...
  const contextStack = []; // { type: 'pool'|'lane', id: string }
...
export function validateBpmnBeta(bpmnBetaCode) {
  ...
  if (pools.length > 1) {
    ...
    for (const flow of flows) {
      if (flow.kind === 'message') continue; // message flows are allowed to cross
  ...
  // VR-009: Balanced brackets — covered by parseErrors (unclosed block detection)
  ...
  for (const andGw of andGateways) {
    ...
    if (outgoing.length >= 2) {
      // This is an AND split — look for a corresponding AND join
      ...
      if (andJoins.length === 0) {
        // Check if all paths end in separate end events
...
// Run as CLI: node scripts/validate-bpmn-beta.mjs [file]
