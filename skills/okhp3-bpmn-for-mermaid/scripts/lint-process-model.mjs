...
// Generic/placeholder names that indicate low-quality labeling
...
export function lintProcessModel(bpmnBetaCode) {
  ...
  // 1. accTitle present and meaningful
  if (!accTitle) {
    issues.push({
      type: 'accessibility',
  ...
  // 2. Gateways have meaningful labels
  ...
  // 3. Tasks have descriptive labels
  ...
  // 4. No more than MAX_LANE_ELEMENTS in a single lane
  ...
  // 5. Parallel gateways have matching joins (quality hint, not structural error)
  ...
  // 6. Process has at least one decision point (informational only for complex processes)
  ...
  // ── Score ────────────────────────────────────────────────────────────────────
  // Score from 100, deducting for each issue by severity
