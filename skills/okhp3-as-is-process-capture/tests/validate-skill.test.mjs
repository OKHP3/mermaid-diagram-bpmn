/**
 * validate-skill.test.mjs — okhp3-as-is-process-capture
 */
...
test('assignStepIds exports named function', async () => {
  const mod = await import(pathToFileURL(join(SKILL_ROOT, 'scripts/assign-step-ids.mjs')).href);
  assert.equal(typeof mod.assignStepIds, 'function');
});

test('assignStepIds returns { valid, errors, warnings, steps }', async () => {
  const { assignStepIds } = await import(pathToFileURL(join(SKILL_ROOT, 'scripts/assign-step-ids.mjs')).href);
