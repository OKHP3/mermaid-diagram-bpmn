...
description: "Design a target-state process and a structured change strategy from a gap analysis. Use this skill when the user wants to redesign a process after gaps have been identified; when they ask \\\"what should the process look like\\\", \\\"how do we fix this\\\", \\\"design the to-be process\\\", or \\\"build a change plan\\\". This is a recommended extension skill — use after process-gap-and-exception-analysis has produced a gap analysis. Do not use it to document only the current state — that is as-is-process-capture; this skill designs what should change, not what exists today. Scope firewall applies: never include employer-proprietary constraints without explicit user authorisation. Produces a future-state process artifact and a change strategy document."
license: "MIT"
...
Version 0.2.0 (this pass) added the `compatibility` declaration, the script fallback instruction, and a sharper discovery-time boundary against `as-is-process-capture`. Classified minor per the versioning table, not patch. No regression suite exists to run before this bump; that limitation is disclosed, not implied away.
