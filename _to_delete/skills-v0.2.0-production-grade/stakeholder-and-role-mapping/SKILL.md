...
- User wants a one-page process summary rather than an engagement register: use `sipoc-generation`
...
Version 0.2.0 (this pass) added the `compatibility` declaration, the script fallback instruction, and a discovery-time boundary against `sipoc-generation` (reciprocal to that skill's own boundary against this one). Classified minor per the versioning table, not patch. No regression suite exists to run before this bump; that limitation is disclosed, not implied away.
