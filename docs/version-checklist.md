...
- [x] `registerExternalDiagrams()` integration — detector, parser, DB accessor, renderer all registered
  _(verified: `bpmn-plugin-integration.test.ts` — 16 assertions against real `mermaid@11.17.2`; runs in merge-blocking CI gate)_
...
- [x] Works when loaded from a browser CDN via native ESM against the tested Mermaid build
  _Exact pair `mermaid@11.17.2` + `@okhp3/mermaid-diagram-bpmn@0.1.1` is covered by `app/public/browser-cdn-example.html`, Chromium E2E, and `pnpm run check:browser-cdn` (September 20 compatibility refresh). Legacy `<script>` tags and arbitrary versions remain unsupported._
