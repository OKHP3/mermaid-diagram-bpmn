...
test('prints observed and canonical totals when the published count matches', () => {
  ...
  writeFileSync(canonPath, JSON.stringify({ ...canon, testCount: 839 }));
  ...
  assert.match(result.stdout, /Observed Vitest tests: 839/);
  assert.match(result.stdout, /Canonical published tests: 839/);
  assert.match(result.stdout, /839 Vitest tests match the published canonical count/);
});
...
test('fails with an actionable message when the canonical count is stale', () => {
  ...
  assert.match(`${result.stdout}${result.stderr}`, /Observed Vitest tests: 839/);
  assert.match(`${result.stdout}${result.stderr}`, /Canonical published tests: 838/);
  assert.match(
    `${result.stdout}${result.stderr}`,
