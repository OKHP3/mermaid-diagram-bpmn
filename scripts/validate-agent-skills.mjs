...
function fail(skillPath, rule, message) {
  failures.push({ skillPath, rule, message });
}
...
for (const entry of readdirSync(skillsRoot, { withFileTypes: true })) {
  ...
  const skillPathLabel = `.agents/skills/${entry.name}/SKILL.md`;
  checked += 1;
  if (!existsSync(skillPath)) {
    if (childEntries.length === 0 || !containsFile(skillDir)) continue;
    fail(skillPathLabel, 'SKILL.md', 'SKILL.md is required (lowercase skill.md is not portable)');
    continue;
  }
  ...
  if (!fm) {
    fail(skillPathLabel, 'frontmatter', 'missing or invalid frontmatter');
    continue;
  }

  const name = getSkillField(fm, 'name');
  const description = getSkillField(fm, 'description');
  if (name !== entry.name) fail(skillPathLabel, 'frontmatter.name', `name must match directory, got ${JSON.stringify(name)}`);
  if (!namePattern.test(name)) fail(skillPathLabel, 'frontmatter.name', 'name is not lowercase hyphenated');
  if (description.length < 50 || description.length > 1024) {
    fail(skillPathLabel, 'frontmatter.description', 'description must be 50-1024 characters');
  }
  if (content.split(/\r?\n/).length > 500) fail(skillPathLabel, 'SKILL.md', 'SKILL.md exceeds 500 lines');

  for (const [key, value] of Object.entries(fm.metadata)) {
    if (typeof value !== 'string') fail(skillPathLabel, `frontmatter.metadata.${key}`, 'metadata value must be a string');
  }
  ...
  for (const ref of refs) {
    if (ref.includes('*')) {
      ...
      if (!existsSync(parent) || readdirSync(parent).every((file) => !file.endsWith(pattern))) {
        fail(skillPathLabel, 'references', `wildcard reference has no matching file ${ref}`);
      }
    } else if (!existsSync(join(skillDir, ref))) {
      fail(skillPathLabel, 'references', `missing referenced file ${ref}`);
    }
  }
  ...
  if (existsSync(scriptsDir)) {
    for (const script of readdirSync(scriptsDir).filter((file) => file.endsWith('.mjs'))) {
      ...
      if (/from\s+["']react["']|from\s+["']react-dom["']|\bwindow\.addEventListener\s*\(/.test(source)) {
        fail(skillPathLabel, `scripts/${script}`, 'contains browser UI code');
      }
    }
  }
}
...
if (failures.length) {
  for (const { skillPath, rule, message } of failures) {
    console.error(`✖ ${skillPath} — ${rule}: ${message}`);
  }
  process.exitCode = 1;
} else {
